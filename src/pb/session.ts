...
import {
  ...
  requestSourceToJSON,
  SystemEventType,
  systemEventTypeFromJSON,
  systemEventTypeToJSON,
  TargetType,
  targetTypeFromJSON,
  targetTypeToJSON,
  UserRole,
...
export const SessionTemplate = {
  ...
  fromJSON(object: any): SessionTemplate {
    return {
      ...
      targetType: isSet(object.targetType) ? targetTypeFromJSON(object.targetType) : 0,
  ...
  toJSON(message: SessionTemplate): unknown {
    ...
    if (message.targetType !== 0) {
      obj.targetType = targetTypeToJSON(message.targetType);
    }
    if (message.source !== "") {
...
function createBaseRoomTemplate(): RoomTemplate {
  return { roomId: "", ownerId: "", name: "", timestamp: 0, maxCount: 0, userList: [] };
}
...
export const RoomTemplate = {
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        ...
        case 7:
          ...
          message.userList.push(UserTemplate.decode(reader, reader.uint32()));
          continue;
      }
      if ((tag & 7) === 4 || tag === 0) {
        break;
  ...
  fromJSON(object: any): RoomTemplate {
    return {
      ...
      userList: globalThis.Array.isArray(object?.userList)
        ? object.userList.map((e: any) => UserTemplate.fromJSON(e))
        : [],
    };
  },
  ...
  toJSON(message: RoomTemplate): unknown {
    ...
    if (message.userList?.length) {
      obj.userList = message.userList.map((e) => UserTemplate.toJSON(e));
    }
    return obj;
  },
  ...
  fromPartial<I extends Exact<DeepPartial<RoomTemplate>, I>>(object: I): RoomTemplate {
    ...
    message.userList = object.userList?.map((e) => UserTemplate.fromPartial(e)) || [];
    return message;
  },
};
...
export const SystemTemplate = {
  ...
  fromJSON(object: any): SystemTemplate {
    return {
      type: isSet(object.type) ? systemEventTypeFromJSON(object.type) : 0,
      source: isSet(object.source) ? globalThis.String(object.source) : "",
  ...
  toJSON(message: SystemTemplate): unknown {
    const obj: any = {};
    if (message.type !== 0) {
      obj.type = systemEventTypeToJSON(message.type);
    }
    if (message.source !== "") {
      obj.source = message.source;
    }
